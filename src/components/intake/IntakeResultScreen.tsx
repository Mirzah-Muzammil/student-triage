...
import { submitFollowUp, type ApiResponse } from "@/services/submit/api";
...
export default function IntakeResultScreen({
  ...
  async function handleFollowUpSubmit(e: FormEvent) {
    e.preventDefault();
    if (!response.requestId) {
      setFollowUpError(
        "We could not find the original request. Please submit a new request.",
      );
      return;
    }
    ...
    try {
      const nextResponse = await submitFollowUp({
        ...
        <div className="bg-red-50 border border-red-200 p-5 rounded-2xl flex items-start gap-4 shadow-sm ">
          ...
          <div>
            ...
            <p className="text-xs text-red-700 font-semibold mt-1 leading-relaxed">
              ...
              <strong className="font-bold">999</strong> immediately. For 24/7
              confidential emotional support, call{" "}
              <strong className="font-bold">Samaritans on 116 123</strong>.
            </p>
          </div>
        </div>
      )}
        ...
        >
          <div>
            ...
            <textarea
              ...
              className="mt-2 min-h-32 w-full resize-y rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700 outline-none transition focus:border-blue-300 focus:bg-white focus:ring-4 focus:ring-blue-50 disabled:cursor-not-allowed disabled:opacity-60"
              placeholder="Add the details we asked for..."
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button
              type="submit"
              disabled={isSubmittingFollowUp}
              className="flex h-10 items-center justify-center rounded-xl bg-blue-600 px-6 text-xs font-bold uppercase tracking-wider text-white shadow-sm transition-all hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <button
              ...
              onClick={onReset}
              disabled={isSubmittingFollowUp}
              className="flex h-10 items-center justify-center rounded-xl border border-slate-200 px-6 text-xs font-bold uppercase tracking-wider text-slate-700 shadow-sm transition-all hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
              New Request
            </button>
          </div>
